...
이 프로젝트는 사내 사용을 위한 독점적 비공개 소프트웨어다. 저장소 루트의 `LICENSE`를 적용한다.

- 승인되지 않은 외부 공개와 배포를 금지한다.
- Third-party Dependency는 각각의 License를 별도로 준수한다.
- 회사 명의 운영이나 조직 이전 전 저작권 귀속과 사내 정책을 확인한다.
- Source Code, Export 원본, 자격증명, NetBox Data를 Public Repository에 포함하지 않는다.
